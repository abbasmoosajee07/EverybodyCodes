...
class Plants:
    ...
    def calculate_brightness(self, test_network = None):
        ...
        all_plants = list(test_network.keys())
        for plant in all_plants:
            ...
            if "free" in plant_root.keys():
                ...
                for branch, thickness in plant_root.items():
                    ...
                    # print(branch, thickness, branch_energy, total_energy)
    ...
    def run_case_studies(self):
        ...
        for study_no, study in enumerate(self.test_cases):
            test_network = og_network.copy()
            for plant, change in enumerate(study, 1):
                test_network[plant]["free"] = change
            study_results = self.calculate_brightness(test_network)
            # print(study_no, study_results)
            results[study_no] = study_results[len(study_results)]
        return results

    def maximise_energy(self):
        case_studies = self.run_case_studies()
        max_base_study = max(case_studies.items(), key=lambda x: x[1])
        print(self.root_network)
        test_study = deepcopy(self.root_network)
        # print(max_base_study)
        # print(case_studies)
        best = [0] * len(self.test_cases[0])

        # for branches in test_study.values():
